...
class SOM(BaseEstimator):
    ...
    def __init__(self, mapsize=None, norm_method='var', initmethod='pca', neigh='Guassian', predict_n_neighbours=5,
                 predit_neighbour_weighting='distance'):
        """
        ...
        self.calc_map_dist()
        self.neigh = neigh
        self.predict_n_neighbours = predict_n_neighbours
        self.predit_neighbour_weighting = predit_neighbour_weighting
    ...
    def fit(self, X, y):
        ...
        self.data_raw = X
        if self.norm_method == 'var':
            self.data = normalize(X, method=self.norm_method)
        else:
            self.data = X
        ...
        self.set_topology(mapsize=self.mapsize)
        self.set_algorithm(initmethod=self.initmethod)
        self.calc_map_dist()
        self.set_data_labels(y) # slow? necessary?

        self.train(trainlen=None, n_job=1, shared_memory='no',verbose='on')
    ...
    def predict(self, X):
        ...
        clf = neighbors.KNeighborsRegressor(self.predict_n_neighbors, weights=self.predit_neighbour_weighting)
        clf.fit(X_train, Y_train)
        # the codebook values are all normalized
        #we can normalize the input data based on mean and std of original data
        X = normalize_by(data_raw[:,:Target], X, method='var')
    ...
    def set_data_labels(self, dlabel):
        if dlabel == None:    
            try:
            ...
            try:
                ...
                pass
                print('no data yet: plesae first set training data to the SOM')
    ...
    def view_map(self, what = 'codebook', which_dim = 'all', pack= 'Yes', text_size = 2.8,save='No', save_dir = 'empty',grid='No',text='Yes',cmap='None',COL_SiZe=6):
        ...
        if np.min(mapsize) >1:
            if pack == 'No':
                ...
                view_2d_Pack(self, text_size, which_dim = which_dim,what = what,save = save, save_dir = save_dir,
                             grid=grid,text=text,CMAP=cmap,col_sz=COL_SiZe)
    ...
    def train(self, X, trainlen=None, n_job=1, shared_memory='no',verbose='on'):
        t0 = time()
        ...
        ########################################
        #rough training
        self.batchtrain(X, njob=n_job, phase='rough', verbose=verbose)
        #######################################
        #Finetuning
        self.batchtrain(X, njob=n_job, phase='finetune', verbose=verbose)
...
def batchtrain(self, X, njob=1, phase=None, verbose='on'):
    t0 = time()

    # seting the parameters
    mn = np.min(self.mapsize)
    if mn == 1:
        mpd = float(self.nnodes*10)/float(self.dlen)
    else:
        mpd = float(self.nnodes)/float(self.dlen)
    
    ms = max(self.mapsize[0], self.mapsize[1])
    if mn == 1:
        ms = ms/2.
...
#             radiusfin = max(1, radiusin/2.)
    ...
    for i in range(trainlen):
        if neigh_func == 'Guassian':
            ...
            H = np.exp(-1.0*self.UD2/(2.0*radius[i]**2)).reshape(self.nnodes, self.nnodes)
        if neigh_func == 'Bubble':
            # in case of Bubble function
#             print radius[i], UD2.shape
#             print UD2
            H = l(radius[i],np.sqrt(self.UD2.flatten())).reshape(self.nnodes, self.nnodes) + 1e-12
#             print H
        
        t1 = time()
        bmu = None
        bmu = self.para_bmu_find(X, self.codebook, njb = njob)
        if verbose=='on':
            print
        #updating the codebook
        t2 = time()
        self.codebook = self.update_codebook_voronoi(X, bmu, H, radius)
